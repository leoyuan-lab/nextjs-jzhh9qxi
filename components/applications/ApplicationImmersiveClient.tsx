...
import type { ApplicationScenarioMedia } from '@/data/application-media';
import {
  clearManufacturingNavTone,
  dispatchManufacturingNavTone,
} from '@/lib/application-nav-tone';
import type {
  ApplicationImmersiveConfig,
  ApplicationImmersiveCopy,
} from '@/lib/application-immersive-config';
...
function useApplicationStoryScroll(trackRef: RefObject<HTMLElement | null>): {
...
function StoryActTwo({ copy }: { copy: ApplicationImmersiveCopy }) {
  return (
...
type CellItem = {
  media: ApplicationScenarioMedia;
  copy: ApplicationImmersiveCopy['story']['step3']['items'][number] | undefined;
};
                  ...
                  {media.kind === 'video' ? (
                    ...
                    <Image
                      ...
                      alt={resolveAlt(
                        safeLang,
                        media.altKey,
                        media.altNamespace === 'variant_images' ? 'variant_images' : 'root',
                      )}
                      fill
...
export function ApplicationImmersiveClient({ config }: { config: ApplicationImmersiveConfig }) {
  const lang = useSiteLang();
  const safeLang: 'zh' | 'en' = lang === 'en' ? 'en' : 'zh';
  const copy = getMessages(safeLang).pages[config.copyKey] as ApplicationImmersiveCopy;
  const reduceMotion = useReducedMotion() === true;
  const { media } = config;
  const specSecondary = config.specSecondary;
  ...
  const { videoProgress, act2Opacity } = useApplicationStoryScroll(storyTrackRef);
  ...
  const cellItems = useMemo(
    () =>
      media.scenarios.map((scenario, index) => ({
        media: scenario,
        copy: copy.story.step3.items[index],
      })),
    [copy.story.step3.items, media.scenarios],
  );
  ...
  const specColumnRight =
    specSecondary === 'rUltra'
      ? (copy.specSheet.columns as { rLite: string; rUltra: string }).rUltra
      : (copy.specSheet.columns as { rLite: string; rCore: string }).rCore;
  const specCtaRight =
    specSecondary === 'rUltra'
      ? (copy.specSheet as { ctaUltra: string }).ctaUltra
      : (copy.specSheet as { ctaCore: string }).ctaCore;
  ...
  const openApplicationInquiry = (model?: (typeof config.inquiryModels)[number]) =>
    openInquiry({
      source: config.inquirySource,
      body:
        safeLang === 'zh'
          ? `行业：${config.industryZh}${model ? `\n意向机型：${model}` : ''}\n负载：\n臂展：\n节拍/周期：\n`
          : `Industry: ${config.industryEn}${model ? `\nModel interest: ${model}` : ''}\nPayload:\nReach:\nCycle time:\n`,
    });

  const openerTitleLines =
    copy.opener.titleLines && copy.opener.titleLines.length >= 2
      ? copy.opener.titleLines
      : null;
  const openerTitleAria =
    openerTitleLines?.join(safeLang === 'zh' ? '，' : ' ') ?? copy.opener.title;
  const openerTitleSep = safeLang === 'zh' ? '，' : ' ';

  return (
    <article
      className={`app-mfg-page app-mfg-page--${safeLang}`}
      aria-label={copy.pageAria}
    >
      <section className="app-mfg-act app-mfg-opener" aria-labelledby="app-mfg-opener-title">
        <div className={`app-mfg-opener-inner${openerVisible ? ' is-visible' : ''}`}>
          ...
          <h1
            id="app-mfg-opener-title"
            className="app-mfg-opener-title"
            aria-label={openerTitleAria}
          >
            {openerTitleLines ? (
              openerTitleLines.map((line, index) => (
                <Fragment key={line}>
                  {index > 0 ? (
                    <span className="app-mfg-opener-title-sep" aria-hidden>
                      {openerTitleSep}
                    </span>
                  ) : null}
                  <span className="app-mfg-opener-title-part">{line}</span>
                </Fragment>
              ))
            ) : (
              copy.opener.title
              ...
              <div className="app-mfg-spec-compare">
                  ...
                  return (
                    <div key={product.id} className="app-mfg-spec-compare-model">
                      ...
                      <h3 className="app-mfg-spec-compare-name">
                        {isLite ? copy.specSheet.columns.rLite : specColumnRight}
                      </h3>
                    </div>
                  );
                })}
                {copy.specSheet.rows.map((row) => (
                  <div key={row.label} className="app-mfg-spec-compare-row">
                    ...
                    <div className="app-mfg-spec-compare-value">
                      <span className="sr-only">{row.label}</span>
                      {(row as { rLite: string; rUltra?: string; rCore?: string })[specSecondary]}
                    </div>
                  </div>
                ))}
                <button
                  ...
                  onClick={() => openApplicationInquiry(config.inquiryModels[0])}
                >
                ...
                <button
                  type="button"
                  className="app-mfg-spec-col-link"
                  onClick={() => openApplicationInquiry(config.inquiryModels[1])}
                >
                  {specCtaRight}
                </button>
              </div>
          ...
          <motion.div className="app-mfg-closing-inner" {...(reduceMotion ? {} : fadeUp)}>
            ...
            <div className="app-mfg-closing-actions">
              <button
                ...
                onClick={() => openApplicationInquiry()}
              >
